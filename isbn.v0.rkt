...
#reader(lib "htdp-intermediate-lambda-reader.ss" "lang")((modname isbn.v0) (read-case-sensitive #t) (teachpacks ()) (htdp-settings #(#t constructor repeating-decimal #f #t none #f () #f)))
;; -----------------------------------------------------------------------------
;; isbn.v0.rkt
;; -----------------------------------------------------------------------------
...
(define (isbn-find str format)
  (local [(define p? (isbn-format->predicate? format))
          (define isbns (filter p? (isbn-find/list str)))]
    (cond
      [(empty? isbns) #f]
      [else (first isbns)])))
...
(define (isbn-find/list str)
  (local [;; [List-of String]
          ...
          (define words/line (map remove-isbn-spaces/list lines))
          
          ;; [List-of String]
          (define words (foldr append '() words/line))]
    (filter isbn? (map remove-isbn-hyphens words))))
...
(define (isbn-word? str) (andmap isbn-letter? (explode str)))
